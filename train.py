...
# ============================ step 2/5 模型 ============================
net=MobileNetV3_large(num_classes=143)
# net=MobileNetV3_small(num_classes=143)
...
scheduler= torch.optim.lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.5) #有序学习率下降，lr*gamma
# ============================ step 5/5 训练 ============================
# 记录每一次的数据，方便绘图
train_curve=list()
valid_curve=list()
...
for epoch in range(MAX_EPOCH):
    ...
    if epoch % 2 == 0:
        ...
        correct_val=0.
        total_val=0.
        for i_,data_ in enumerate(valid_loader):
            img_,label_=data_
            img = Variable(img_)
            label = Variable(label_)
            if torch.cuda.is_available():
                img=img.cuda()
                label=label.cuda()
            out_=net(img)
            ...
            print("每五步验证如下：")
            print('epoch:{},val_loss:{:.4f}'.format(epoch+1,loss.data.item()))
            _, predicted_ = torch.max(out_.data, 1)
            total_val += label.size(0)
            correct_val += (predicted_ == label).sum()

    scheduler.step()
    print("============================================")
    accurancy=correct / total
    accurancy_val = correct_val/total_val
